export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({
      status: 'active',
      service: 'Cakto Webhook Integration Endpoint',
  ...
  try {
    ...
    // Parse Cakto real payload format
    const event = (body.event || body.webhook_event_type || '').toLowerCase();
    ...
    let tier = 'VIP';
    if (productName.includes('free') || (data.amount && parseFloat(data.amount) === 0)) {
      tier = 'Free';
    }
    ...
    if (!email) {
      return res.status(400).json({
        success: false,
        error: 'Email do aluno não informado no payload enviado pela Cakto.'
      });
    }
    ...
    // Upsert into Supabase database table profiles
    ...
    if (!dbResponse.ok) {
      ...
      console.error('Supabase HTTP Error:', errText);
    }
    ...
    return res.status(200).json({
      ...
      gateway: 'cakto',
      message: isRefunded 
        ? `Acesso do aluno ${email} foi revogado/bloqueado via Webhook Cakto.`
    ...
    return res.status(500).json({
      success: false,
      error: error.message || 'Erro interno ao processar Webhook da Cakto'
    });
  }
}
