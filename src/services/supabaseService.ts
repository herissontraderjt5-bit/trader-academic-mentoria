  ...
  async getTelegramSignalSettings(): Promise<TelegramSignalSettings | null> {
    ...
    try {
      ...
      return {
        ...
        startMessageTemplate: data.start_message_template || 'Bom dia Traders! Iniciando as operações.',
        endMessageTemplate: data.end_message_template || 'Fim das operações da sessão.',
        dailyResultMessageTemplate: data.daily_result_message_template || 'RESULTADO DO DIA!\nWins: {WINS}\nLosses: {LOSSES}',
        preAlertMessageTemplate: data.pre_alert_message_template || 'Atenção! Possível sinal de {DIRECTION} em {TICKER} - {TIMEFRAME}',
  ...
  async saveTelegramSignalSettings(settings: TelegramSignalSettings): Promise<boolean> {
    ...
    try {
      const { error } = await supabase.from('telegram_signal_settings').upsert({
        ...
        daily_result_message_template: settings.dailyResultMessageTemplate,
        pre_alert_message_template: settings.preAlertMessageTemplate,
        confirmation_message_template: settings.confirmationMessageTemplate,
        pre_alert_minutes: settings.preAlertMinutes,
        martingale_level: settings.martingaleLevel,
